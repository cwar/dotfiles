    ...
    for themes_dir in (STOCK_THEMES_DIR, USER_THEMES_DIR):
        ...
        for entry in themes_dir.iterdir():
            if entry.is_dir() or entry.is_symlink():
                themes.add(entry.name.lower())
    return themes


def search_github(query: str = "omarchy theme", sort: str = "stars", per_page: int = 50) -> list[dict]:
...
def run_interactive(results: dict, multi: bool = False):
    ...
    fzf_args = [
        ...
        "●",
        "--color",
        "bg+:#313244,fg+:#cdd6f4,hl:#f38ba8,hl+:#f38ba8,info:#89b4fa,prompt:#89b4fa,pointer:#f5c2e7,marker:#a6e3a1,header:#89b4fa,border:#585b70",
        "--border",
        "rounded",
        "--border-label",
        " 🎨 Omarchy Theme Discovery ",
        "--margin",
        "1,2",
        "--padding",
        "1,0",
...
def print_results(results: dict):
    ...
    print(
        f"  Found {results['total_found']} themes total ("
