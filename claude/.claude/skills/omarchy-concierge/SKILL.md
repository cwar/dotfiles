...
**IMPORTANT: Do NOT use wrapper scripts in Hyprland `exec` bindings.** Custom scripts in `~/.local/bin/` called from Hyprland's `exec` dispatcher silently fail due to how `uwsm-app` and `setsid` interact with the Wayland session context. Instead, put all logic inline in the binding or use shell expansion:

```bash
# BAD - wrapper script will silently fail
bindd = SUPER SHIFT, G, Lazygit, exec, my-custom-script

# GOOD - use omarchy commands directly with inline shell expansion
bindd = SUPER SHIFT, G, Lazygit, exec, omarchy-launch-tui lazygit -p "$(omarchy-cmd-terminal-cwd)"
```

**Launching TUI apps with `omarchy-launch-tui`:** This command accepts extra arguments after the command name, which are passed through to the TUI app. Use `omarchy-cmd-terminal-cwd` to get the active terminal's working directory for commands that need it:

```bash
# Launch lazygit in the active terminal's working directory
bindd = SUPER SHIFT, G, Lazygit, exec, omarchy-launch-tui lazygit -p "$(omarchy-cmd-terminal-cwd)"
```
